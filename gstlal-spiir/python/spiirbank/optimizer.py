# Copyright (C) 2017-2018 Joel Bosveld, David Weight
#
...
import numpy as np
from optimizer_cy import calc_y_dy_ddy, calc_Y, calc_dY, calc_ddY, calc_ddYd
from optimizer_cy import recalc_y_dy_ddy, recalc_Y, recalc_dY, recalc_ddY
from optimizer_cy import chol_update, chol_downdate
...
def initialize_state(template, n):
    ...
    state = (template, a1_conj, y, dy, ddy, Y, dY, ddY, ddYd, fact, b0, norm_a,
             overlap_a)
    return state
...
def chol_resync(state):
    (template, a1_conj, y, dy, ddy, Y, dY, ddY, ddYd, fact, b0, norm_a,
     overlap_a) = state
    tmp = la.cho_factor(Y, lower=fact[1])
    assert (np.sum(np.abs(
        (tmp[0] - fact[0])[np.tril_indices(len(a1_conj))])) < 1e-9)
    fact[0][:, :] = tmp[0]
...
def calc_do2(a1, delay, state):
    ...
    for i in range(len(a1)):
        do2[i] = 2 * (np.conj(b0[i]) * dy[i] -
                      np.conj(b0[i]) * np.dot(np.conj(dY[:, i]), b0))
    return do2
...
def calc_do2_indv(a1, delay, state, j):
    ...
    do2 = 2 * (np.conj(b0[j]) * dy[j] -
               np.conj(b0[j]) * np.dot(np.conj(dY[:, j]), b0))
...
def calc_ddo2(a1, delay, state):
    ...
    for fi in range(2 * len(a1)):
        for fj in range(fi, 2 * len(a1)):
            ...
            bhddYdbh = np.conj(b0[j]) * np.dot(np.conj(ci * cj * ddYd[:, j]),
                                               b0) if i == j else 0
            dYibh = dYbh[fi % 2][:, i]
            dYjbh = dYbh[fj % 2][:, j]
            ...
            ddo2[fi, fj] = 2 * (np.conj(b0[j]) * ddyij +
...
def calc_ddo2_indv(a1, delay, state, j):
    ...
    for fi in range(2):
        for fj in range(fi, 2):
            ...
            ddo2[fi, fj] = 2 * (
                np.conj(b0[j]) * ddyij + np.conj(dy[j] * ci) * cj * Yinvdy[j] -
                np.dot(np.conj(dYjbh * ci), Yinvdy) -
                np.dot(np.conj(dYibh * cj), Yinvdy) +
                np.dot(np.conj(dYjbh), YinvdYibh) -
                np.conj(b0[j] * cj) * ddY[j, j] * b0[j] * ci - bhddYdbh).real
            ddo2[fj, fi] = ddo2[fi, fj]
...
def saddle_free_newton(do, ddo, lam=0):
    ...
    return np.dot(
        vec,
        np.multiply(1 / (np.abs(val) + lam),
                    np.dot(
                        vec.T,
                        do.view(dtype=np.float64)))).view(dtype=np.complex128)
