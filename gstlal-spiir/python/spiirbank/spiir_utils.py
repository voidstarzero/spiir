...
def gen_whitened_fir_template(template_table,
    ...
    working_length = cbc_template_iir.ceil_pow_2(length_max +
                                                 round(1. / psd.deltaF *
                                                       sample_rate_max))
    ...
    template_bank = [
        numpy.zeros(
            (2 * len(template_table), int(round(rate * (end - begin)))),
            dtype="double") for rate, begin, end in time_slices
    ]
    ...
    fseries = cbc_template_fir.generate_template(row,
                                                 approximant,
                                                 sample_rate_max,
                                                 working_duration,
                                                 f_low,
    ...
    autocorrelation_bank_full = numpy.zeros(autocorrelation_length,
                                            dtype="cdouble")
    ...
    if approximant in gstlal_IMR_approximants:
        ...
        row.end = lal.LIGOTimeGPS(
            float(target_index - (len(data_full) - 1.)) / sample_rate_max)
    else:
    ...
    autocorrelation_bank = normalized_crosscorr(data_full, data,
...
def gen_whitened_spiir_template_and_reconstructed_waveform(
    ...
    working_state = gen_template_working_state(sngl_inspiral_table,
                                               flower,
    ...
    if verbose:
        ...
        logging.info(
            ...
            (working_state["working_f_low"], working_state["working_duration"],
             flower, sampleRate))
    ...
    this_tchirp = lalsimulation.SimInspiralChirpTimeBound(
        flower, row.mass1 * lal.MSUN_SI, row.mass2 * lal.MSUN_SI, row.spin1z,
        row.spin2z)

    if verbose:
        logging.info("working_duration %f, chirp time %f" %
    ...
    while (spiir_match < req_min_match and epsilon > epsilon_min
           and n_filters < 2000):
        ...
        spiir_match = abs(
            numpy.dot(u_rev_pad, numpy.conj(h_pad)) /
            numpy.sqrt(norm_u * norm_h))
