...
def ncx2logpdf(x, k, l):
    return -math.log(2.) - (x + l) / 2. + (k / 4. - .5) * (
        numpy.log(x) - numpy.log(l)) + logiv(k / 2. - 1.,
                                             numpy.sqrt(l) * numpy.sqrt(x))
...
def gen_signal_stats(ncx2_dof=2, ncx2_mean_factor=0.045):
    sgstats_pdf_mat = numpy.zeros((pipe_macro.xbin, pipe_macro.ybin),
                                  dtype=numpy.float64)
    sgstats_rate_mat = numpy.zeros((pipe_macro.xbin, pipe_macro.ybin),
                                   dtype=numpy.int64)
    lgchisq_tick = numpy.linspace(pipe_macro.ymin, pipe_macro.ymax,
                                  pipe_macro.ybin)
    lgsnr_tick = numpy.linspace(pipe_macro.xmin, pipe_macro.xmax,
                                pipe_macro.xbin)
    for i_snr in range(0, pipe_macro.xbin):
        for i_chisq in range(0, pipe_macro.ybin):
            snr = 10**lgsnr_tick[i_snr]
            sgstats_pdf_mat[i_chisq, i_snr] = ncx2pdf(
                10**lgchisq_tick[i_chisq], ncx2_dof,
                1 + snr * snr * ncx2_mean_factor * ncx2_mean_factor)
            sgstats_rate_mat[i_chisq, i_snr] = math.floor(
                1e10 * sgstats_pdf_mat[i_chisq, i_snr])
...
def signal_stats_to_xml(filename,
                        ifos,
                        ncx2_dof=2,
                        ncx2_mean_factor=0.045,
                        verbose=False):
    ...
    mat_zero_double = numpy.zeros((pipe_macro.xbin, pipe_macro.ybin),
                                  dtype=numpy.float64)
