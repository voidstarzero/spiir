...
def get_connection_filename(filename,
                            tmp_path=None,
                            replace_file=False,
                            verbose=False):
    """
    ...
    if tmp_path is not None:
        ...
        with temporary_files_lock:
            if filename in temporary_files:
                raise ValueError(
                    "file '%s' appears to be in use already as a temporary database file and is to be deleted"
                    % filename)
        target = filename
...
def idmap_create(connection):
    ...
    connection.cursor().execute(
        "CREATE TEMPORARY TABLE _idmap_ (old TEXT PRIMARY KEY NOT NULL, new TEXT NOT NULL)"
    )
...
def idmap_get_max_id(connection, id_class):
    ...
    cursor.execute(
        ...
        (id_class.column_name, id_class.index_offset + 1, id_class.table_name))
...
def get_column_info(connection, table_name):
    ...
    return [(coldef.groupdict()["name"], coldef.groupdict()["type"])
            ...
            if coldef.groupdict()["name"].upper() not in ("PRIMARY", "UNIQUE",
                                                          "CHECK")]
...
def get_xml(connection, table_names=None):
    ...
    for table_name in table_names:
        ...
        for column_name, column_type in get_column_info(
                connection, table_elem.Name):
...
# NOTE: add postcoh info to __new__ method
class DBTable(table.Table):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not hasattr(cls, "tableName"):
            ...
            if name in postcoh_table_def.TableByName:
                # found metadata in postcohtable, construct
                # custom subclass.  the class from
                # postcohtable is added as a parent class to
                # allow methods from that class to be used
                # with this class, however there is no
                # guarantee that all parent class methods
                # will be appropriate for use with the
                # DB-backend object.
                lsccls = postcoh_table_def.TableByName[name]
    ...
    def _end_of_columns(self):
        ...
        if self.loadcolumns is not None:
            ...
            self.dbcolumntypes = [
                name for i, name in enumerate(self.columntypes)
    ...
    def applyKeyMapping(self):
        ...
        assignments = ", ".join(
            ...
            (colname, colname)
            for coltype, colname in zip(self.dbcolumntypes, self.dbcolumnnames)
            if coltype in ligolwtypes.IDTypes and (
                self.next_id is None or colname != self.next_id.column_name))
        if assignments:
            ...
            self.cursor.execute("UPDATE %s SET %s WHERE ROWID > %d" %
                                (self.Name, assignments, self.last_maxrowid))
            self.last_maxrowid = self.maxrowid() or 0
...
class TimeSlideTable(DBTable):
    ...
    def get_time_slide_id(self,
        ...
        if superset_ok:
            ids = [
                id for id, slide in self.as_dict().items()
                if offsetdict == dict((instrument, offset)
