...
def insert_from_url(url,
    ...
    orig_DBTable_append = dbtables_postcoh.DBTable.append

    if not preserve_ids:
        try:
            dbtables_postcoh.idmap_create(contenthandler.connection)
        except sqlite3.OperationalError:
            # assume table already exists
            pass
        dbtables_postcoh.idmap_sync(contenthandler.connection)
        dbtables_postcoh.DBTable.append = dbtables_postcoh.DBTable._remapping_append
    else:
        dbtables_postcoh.DBTable.append = dbtables_postcoh.DBTable._append
    ...
    try:
        ...
        dbtables_postcoh.DBTable.append = orig_DBTable_append
...
def insert_from_xmldoc(connection,
                       source_xmldoc,
                       preserve_ids=False,
                       verbose=False):
    """
...
# NOTE: only change the dbtables_postcoh
