...
class tab(elem):
    ...
    def __call__(self, content=[]):
        return elem("div", content,
                    """ id="%s" class="tabcontent" """ % self.div)
...
class page(object):
    def __init__(
        ...
        if header_content is None:
            header_content = [
                """<script type="text/javascript">google.charts.load('current', {'packages':['table', 'timeline']});</script>"""
            ]
        self.title = title
        self.path = path
        self.css = css
    ...
    def write(self, f, write_gstlal_css=True):
        css_list = [
            elem("link", [],
                 """ rel="stylesheet" type="text/css" href="%s" """ % c)
            for c in self.css
        ]
        script_list = [
            elem("script", [], """ type="text/javascript" src="%s" """ % s)
