 ...
 *		  and dump the informations about the nodes found in the
 *process. (Note that the XMLReader functions require libxml2 version later
 *		  than 2.6.)
...
void readArray(xmlTextReaderPtr reader, void *data) {
    ...
    while (1) {
        ...
        if (xmlStrcmp(name, BAD_CAST "Stream") == 0 && nodeType != 15) {
            ...
            bytes = rows * xArrayPtr->dim[xArrayPtr->ndim - 1]
...
void freeArray(XmlArray *array) { free(array->data); }
...
void readParam(xmlTextReaderPtr reader, void *data) {
    ...
    while (1) {
        ...
        if (xmlStrcmp(name, BAD_CAST "#text") == 0) {
            ...
            // save content
            sscanf((char *)content, ligoxml_get_type_format(type),
                   xmlParamPtr->data);
        }

        // Meet node </Param>, just break out the loop
        if (xmlStrcmp(name, BAD_CAST "Param") == 0 && nodeType == 15) { break; }
    }
}

void freeParam(XmlParam *param) { free(param->data); }
...
void readTable(xmlTextReaderPtr reader, void *data) {
    ...
    while (1) {
        ...
        if (xmlStrcmp(nodeName, BAD_CAST "Column") == 0) {
            ...
            g_array_append_val(xmlTable->names,
                               *g_string_new((const gchar *)columnName));
            ...
            g_hash_table_insert(xmlTable->hashContent,
                                g_string_new((const gchar *)columnName),
                                (gpointer)(val));
        }
        ...
        if (xmlStrcmp(nodeName, BAD_CAST "Stream") == 0 && nodeType != 15) {
            ...
            GString *content =
              g_string_new((const gchar *)xmlTextReaderConstValue(reader));
            ...
            for (i = 1; i <= numLine; ++i) {
                ...
                for (j = 0; j < numCol; ++j) {
                    ...
                    XmlHashVal *valPtr = g_hash_table_lookup(
                      xmlTable->hashContent, (gpointer)colName);
...
void processNode(xmlTextReaderPtr reader,
    ...
    if (xmlTextReaderNodeType(reader) == 15) return;
 ...
 */
void parseFile(const char *filename, XmlNodeStruct *xns, int len) {
