...
void spiir_state_load_bank(SpiirState **spstate,
    ...
    XmlNodeStruct *inxns =
      (XmlNodeStruct *)malloc(sizeof(XmlNodeStruct) * depth * 3);
    ...
    for (i = 0; i < depth; ++i) {
        // configure d_array
        d_array[i].ndim = 0;
        sprintf((char *)inxns[i + 0 * depth].tag, "Array-d_%d:array",
                maxrate >> i);
        ...
        // configure a_array
        a_array[i].ndim = 0;
        sprintf((char *)inxns[i + 1 * depth].tag, "Array-a_%d:array",
                maxrate >> i);
        ...
        // configure b_array
        b_array[i].ndim = 0;
        sprintf((char *)inxns[i + 2 * depth].tag, "Array-b_%d:array",
                maxrate >> i);
    ...
    for (i = 0; i < depth; ++i) {
        ...
        spstate[i]->d_d =
          (int *)malloc(sizeof(int) * num_filters * num_templates);
        printf("%d - d_dim: (%d, %d) a_dim: (%d, %d) b_dim: (%d, %d)\n", i,
               d_array[i].dim[0], d_array[i].dim[1], a_array[i].dim[0],
               a_array[i].dim[1], b_array[i].dim[0], b_array[i].dim[1]);

        spstate[i]->num_filters   = num_filters;
        spstate[i]->num_templates = num_templates;
        spstate[i]->d_a1 =
          (COMPLEX_F *)malloc(sizeof(COMPLEX_F) * num_filters * num_templates);
        spstate[i]->d_b0 =
          (COMPLEX_F *)malloc(sizeof(COMPLEX_F) * num_filters * num_templates);
        for (j = 0; j < num_filters; ++j) {
            for (k = 0; k < num_templates; ++k) {
                spstate[i]->d_d[j * num_templates + k] =
                  ((long *)(d_array[i].data))[j * num_templates + k];
                spstate[i]->d_a1[j * num_templates + k].re =
                  ((double *)(a_array[i].data))[j * 2 * num_templates + k];
                spstate[i]->d_a1[j * num_templates + k].im =
                  ((double *)(a_array[i].data))[j * 2 * num_templates
                                                + num_templates + k];
                spstate[i]->d_b0[j * num_templates + k].re =
                  ((double *)(b_array[i].data))[j * 2 * num_templates + k];
                spstate[i]->d_b0[j * num_templates + k].im =
        ...
        printf("1st a: (%.3f + %.3fi) 1st b: (%.3f + %.3fi) 1st d: %d\n",
               ...
               spstate[i]->d_b0[0].re, spstate[i]->d_b0[0].im,
               spstate[i]->d_d[0]);
    }
...
int main(int argc, char **argv) {
    ...
    /*
       ...
       PROCESSLEN); XmlArray xarray = {0, {1, 1, 1}, NULL}; XmlParam xparam =
       {0, NULL}; XmlTable xtable = {NULL, NULL};
    ...
    */
    ...
    /*
        ...
        for (i = 0; i < xarray.dim[0]; ++i)
        {
            ...
            {
                printf("data[%d][%d] = %.1f ", i, j, data[i * xarray.dim[1] +
       j]);
            }
            printf("\n");
        }
        ...
        printf("\nTable Name: %s; Columns: %u;\n", (xtable.tableName)->str,
       (xtable.names)->len); GHashTable *hash = xtable.hashContent; for (i = 0;
       i < (xtable.names)->len; ++i)
        {
