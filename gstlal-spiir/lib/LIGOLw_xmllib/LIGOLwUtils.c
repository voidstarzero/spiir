...
int ligoxml_get_type_index(const xmlChar *type) {
    int i;
    for (i = 0; i < MAPSIZE; ++i) {
        if (xmlStrcmp(type, typeMap[i].xml_type) == 0) return typeMap[i].index;
    }
...
size_t ligoxml_get_type_size(const xmlChar *type) {
    int i;
    for (i = 0; i < MAPSIZE; ++i) {
        if (xmlStrcmp(type, typeMap[i].xml_type) == 0) return typeMap[i].bytes;
    }
...
const char *ligoxml_get_type_format(const xmlChar *type) {
    int i;
    for (i = 0; i < MAPSIZE; ++i) {
        if (xmlStrcmp(type, typeMap[i].xml_type) == 0) return typeMap[i].format;
    }
