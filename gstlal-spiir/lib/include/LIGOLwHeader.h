...
typedef void (*DataToString)(void *des,
                             const xmlChar *xml_type,
                             void *data,
                             int pos);

void data_to_string_char(void *des,
                         const xmlChar *xml_type,
                         void *data,
                         int pos);
void data_to_string_double(void *des,
                           const xmlChar *xml_type,
                           void *data,
                           int pos);
void data_to_string_float(void *des,
                          const xmlChar *xml_type,
                          void *data,
                          int pos);
void data_to_string_int(void *des,
                        const xmlChar *xml_type,
                        void *data,
                        int pos);
void data_to_string_long(void *des,
                         const xmlChar *xml_type,
                         void *data,
                         int pos);
...
static const XmlTypeMap typeMap[MAPSIZE] = {
    ...
    /* FIXME: read and write should be different.
     * read %lg, write %.16g */
    { BAD_CAST "real_8", data_to_string_double, "%lg", sizeof(double), 1 },
    /* FIXME: read and write should be different.
     * read %g, write %.8g */
    { BAD_CAST "real_4", data_to_string_float, "%g", sizeof(float), 2 },
