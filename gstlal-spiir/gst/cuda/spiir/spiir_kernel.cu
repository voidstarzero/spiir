...
unsigned iir_channels(const GSTLALIIRBankCuda *element) {
    if (element->a1) return 2 * element->a1->size1;
    return 0;
}
...
void set_metadata(GSTLALIIRBankCuda *element,
                  ...
                  gboolean gap) {
    GST_BUFFER_SIZE(buf) =
      outsamples * iir_channels(element) * element->width / 8;
    ...
    GST_BUFFER_TIMESTAMP(buf) =
      ...
      + gst_util_uint64_scale_int_round(
        GST_BUFFER_OFFSET(buf) - element->offset0, GST_SECOND, element->rate);
    GST_BUFFER_DURATION(buf) = element->t0
                               + gst_util_uint64_scale_int_round(
                                 GST_BUFFER_OFFSET_END(buf) - element->offset0,
    ...
    if (gap) GST_BUFFER_FLAG_SET(buf, GST_BUFFER_FLAG_GAP);
    else
...
int cuda_bank_init(GSTLALIIRBankCuda *element) {
    ...
    cudaMemcpyAsync(bank->d_b0_s, h_b0_s, size1 * size2 * sizeof(COMPLEX8_F),
                    cudaMemcpyHostToDevice, element->stream);
    cudaMemcpyAsync(bank->d_y_s, h_y_s, size1 * size2 * sizeof(COMPLEX8_F),
                    cudaMemcpyHostToDevice, element->stream);
...
__global__ void cuda_iir_filter_kernel(COMPLEX8_F *cudaA1,
    ...
    if (tx < numFilters) {
        ...
        if (tx < nb) { snr = &(cudaSnr[by * step_points + tx]); }
...
GstFlowReturn filter_d(GSTLALIIRBankCuda *element, GstBuffer *outbuf) {
    ...
    // streames numbers set to allow processing 5 bank file, 5 files * 3
    detectors * 8 rates = 120 static cudaStream_t streams[128]; //=
    ...
    */
    ...
    if (!output_length) return GST_BASE_TRANSFORM_FLOW_DROPPED;
    ...
    if (output_length > bank->pre_output_length) {
        ...
        if (bank->h_input_s) { cudaFreeHost(bank->h_input_s); }
        cudaMallocHost(&bank->h_input_s, available_length * sizeof(float));
        // bank->h_input_s = (float *)malloc( available_length * sizeof(float));
        if (bank->h_output_s) { cudaFreeHost(bank->h_output_s); }
        cudaMallocHost(&bank->h_output_s, bank->num_templates * output_length
                                            * sizeof(COMPLEX8_F));
        // bank->h_output_s = (COMPLEX8_F *)malloc(bank->num_templates *
        // output_length * sizeof(COMPLEX8_F) );

        // realloc gpu input and output buf
        if (bank->d_input_s) { cudaFree(bank->d_input_s); }
        cudaMalloc((void **)&bank->d_input_s, available_length * sizeof(float));
        if (bank->d_output_s) { cudaFree(bank->d_output_s); }
        cudaMalloc((void **)&bank->d_output_s,
    ...
    uint share_mem_sz =
      (block.x + 8 + (bank->num_filters + 16 - 1) / 16 * bank->nb)
      * sizeof(float) * 2;
    ...
    cudaBindTexture(0, texRef, bank->d_input_s, channelDesc, available_length *
    sizeof(float));
    */
    cuda_iir_filter_kernel<<<grid, block, share_mem_sz, element->stream>>>(
      bank->d_a1_s, bank->d_b0_s, bank->d_d_i, bank->d_y_s, bank->d_input_s,
      bank->d_output_s, bank->num_filters, output_length, bank->dmax, bank->nb);
    // g_mutex_unlock(element->cuTex_lock);

    /*
     * transfer the output data to GPU -> CPU
     */
    cudaMemcpyAsync(bank->h_output_s, bank->d_output_s,
                    output_length * bank->num_templates * sizeof(COMPLEX8_F),
    ...
    g_assert(output_length * iir_channels(element) / 2 * sizeof(complex double)
...
GstFlowReturn filter_s(GSTLALIIRBankCuda *element, GstBuffer *outbuf) {
    ...
    */
    ...
    g_assert(output_length * iir_channels(element) / 2 * sizeof(complex float)
             <= GST_BUFFER_SIZE(outbuf));
