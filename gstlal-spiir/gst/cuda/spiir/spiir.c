...
enum cuda_iirbank_signal { SIGNAL_RATE_CHANGED, NUM_SIGNALS };
...
GST_BOILERPLATE(GSTLALIIRBankCuda,
                cuda_iirbank,
                GstBaseTransform,
                GST_TYPE_BASE_TRANSFORM);

enum property { ARG_IIR_A1 = 1, ARG_IIR_B0, ARG_IIR_DELAY };
...
static gboolean
  get_unit_size(GstBaseTransform *trans, GstCaps *caps, guint *size) {
    ...
    if (success) *size = width / 8 * channels;
    else
        GST_WARNING_OBJECT(
          trans, "unable to parse channels from %" GST_PTR_FORMAT, caps);
...
static GstCaps *transform_caps(GstBaseTransform *trans,
    ...
    case GST_PAD_SRC:
        ...
        for (n = 0; n < gst_caps_get_size(caps); n++)
            gst_structure_set(gst_caps_get_structure(caps, n), "channels",
                              G_TYPE_INT, 1, NULL);
        break;
    ...
    case GST_PAD_SINK:
        ...
        for (n = 0; n < gst_caps_get_size(caps); n++) {
            if (element->delay)
                gst_structure_set(gst_caps_get_structure(caps, n), "channels",
                                  G_TYPE_INT, iir_channels(element), NULL);
            else
                gst_structure_set(gst_caps_get_structure(caps, n), "channels",
...
static gboolean transform_size(GstBaseTransform *trans,
    ...
    if (!get_unit_size(trans, caps, &unit_size)) return FALSE;
    if (size % unit_size) {
        GST_DEBUG_OBJECT(element, "size not a multiple of %u", unit_size);
        return FALSE;
    }
    if (!get_unit_size(trans, othercaps, &other_unit_size)) return FALSE;
    ...
    case GST_PAD_SINK:
        ...
        while (!element->delay || !element->a1 || !element->b0)
            g_cond_wait(element->iir_matrix_available,
                        element->iir_matrix_lock);
...
static gboolean
  set_caps(GstBaseTransform *trans, GstCaps *incaps, GstCaps *outcaps) {
    ...
    if (success && element->delay
        && (channels != (gint)iir_channels(element))) {
        GST_DEBUG_OBJECT(element, "channels != %d in %" GST_PTR_FORMAT,
                         iir_channels(element), outcaps);
        success = FALSE;
    }

    if (success) {
        gboolean format_changed =
          element->rate != rate || element->width != width;
        ...
        if (element->rate != old_rate)
            g_signal_emit(G_OBJECT(trans), signals[SIGNAL_RATE_CHANGED], 0,
                          element->rate, NULL);
...
static GstFlowReturn
  transform(GstBaseTransform *trans, GstBuffer *inbuf, GstBuffer *outbuf) {
    ...
    if (!element->y)
        element->y =
          gsl_matrix_complex_calloc(element->a1->size1, element->a1->size2);
    else if (element->y->size1 != element->delay->size1
             || element->y->size2 != element->delay->size2) {
    ...
    if (GST_BUFFER_IS_DISCONT(inbuf)
        ...
        /*
         * flush adapter. Erase contents of adaptor. Push dmax - dmin zeros, so
         * the IIR filter bank can start filtering from t0.
         */
         ...
         * the timestamp and offset of the inbuf. Next_out_offset is to set the
         * offset of the outbuf.
         */
        ...
        /*
         * be sure to flag the next output buffer as a discontinuity, because it
    ...
    if (!GST_BUFFER_FLAG_IS_SET(inbuf, GST_BUFFER_FLAG_GAP)) {
        ...
        element->zeros_in_adapter = 0;
        if (element->width == 64) result = filter_d(element, outbuf);
    ...
    // GST_BUFFER_TIMESTAMP(inbuf), GST_BUFFER_SIZE(inbuf) / sizeof(double));

    int dmin, dmax;
    gsl_matrix_int_minmax(element->delay, &dmin, &dmax);
    dmin = 0;
    /*fprintf(stderr, "IIR elem %17s : input timestamp %llu, output timestamp
       %llu, input offset %llu, output offset %llu, %d, %d\n",
            ...
            dmin,
            dmax);*/
    // fprintf(stderr, "IIR elem %17s : input offset %llu, output offset %llu,
    // %d, %d\n", GST_ELEMENT_NAME(element), GST_BUFFER_OFFSET(inbuf),
    // GST_BUFFER_OFFSET(outbuf), dmin, dmax);
    /*
     * done
     */
...
static void set_property(GObject *object,
    ...
    case ARG_IIR_A1:
        g_mutex_lock(element->iir_matrix_lock);
        if (element->a1) gsl_matrix_complex_free(element->a1);

        element->a1 = gstlal_gsl_matrix_complex_from_g_value_array(
    ...
    case ARG_IIR_B0:
        g_mutex_lock(element->iir_matrix_lock);
        if (element->b0) gsl_matrix_complex_free(element->b0);

        element->b0 = gstlal_gsl_matrix_complex_from_g_value_array(
          g_value_get_boxed(value));
    ...
    case ARG_IIR_DELAY: {
        ...
        element->delay =
          gstlal_gsl_matrix_int_from_g_value_array(g_value_get_boxed(value));
...
static void get_property(GObject *object,
    ...
    case ARG_IIR_A1:
        ...
        if (element->a1)
            g_value_take_boxed(
              value, gstlal_g_value_array_from_gsl_matrix_complex(element->a1));
        /* FIXME:  else? */
        g_mutex_unlock(element->iir_matrix_lock);
        break;
    ...
    case ARG_IIR_B0:
        ...
        if (element->b0)
            g_value_take_boxed(
              value, gstlal_g_value_array_from_gsl_matrix_complex(element->b0));
        /* FIXME:  else? */
        g_mutex_unlock(element->iir_matrix_lock);
        break;
    ...
    case ARG_IIR_DELAY:
        ...
        if (element->delay)
            g_value_take_boxed(
              value, gstlal_g_value_array_from_gsl_matrix_int(element->delay));
        /* FIXME:  else? */
        g_mutex_unlock(element->iir_matrix_lock);
        break;
...
static void cuda_iirbank_base_init(gpointer gclass) {
    ...
    gst_element_class_set_details_simple(
      element_class, "Cuda Mode IIR Filter Bank", "Filter/Audio",
      "Projects a single audio channel onto a bank of IIR filters to produce a "
      "multi-channel output",
      "Qi Chu <chicsheep@gmail.com>");
...
static void cuda_iirbank_class_init(GSTLALIIRBankCudaClass *klass) {
    ...
    g_object_class_install_property(
      gobject_class, ARG_IIR_A1,
      g_param_spec_value_array(
        "a1-matrix", "Matric of IIR feedback coefficients",
        "A Matrix of first order IIR filter feedback coefficients. Each row "
        "represents a different IIR bank.",
        g_param_spec_value_array(
          "a1", "IIR bank feedback coefficients",
          "A parallel bank of first order IIR filter feedback coefficients",
          g_param_spec_double("coefficient", "Coefficient",
                              "Feedback coefficient", -G_MAXDOUBLE, G_MAXDOUBLE,
                              0.0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
      gobject_class, ARG_IIR_B0,
      g_param_spec_value_array(
        "b0-matrix", "Matrix of IIR bank feedforward coefficients",
        "Array of first order IIR filter coefficients. Each row represents a "
        "different IIR bank.",
        g_param_spec_value_array(
          "b0", "IIR bank of feedforward coefficients",
          "A parallel bank of first order IIR filter feedforward coefficents",
          g_param_spec_double("coefficient", "Coefficient",
                              "Current input sample coefficient", -G_MAXDOUBLE,
                              G_MAXDOUBLE, 0.0,
                              G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(
      gobject_class, ARG_IIR_DELAY,
      g_param_spec_value_array(
        "delay-matrix", "Matrix of delays for IIR filter bank",
        "Matrix of delays for first order IIR filters.  All filters must have "
        "the same length.",
        g_param_spec_value_array(
          "delay", "delays for IIR bank",
          "A parallel bank of first order IIR filter delays",
          g_param_spec_int("delay", "Delay", "Delay for IIR filter", 0,
                           G_MAXINT, 0,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS),
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    signals[SIGNAL_RATE_CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(GSTLALIIRBankCudaClass, rate_changed), NULL, NULL,
