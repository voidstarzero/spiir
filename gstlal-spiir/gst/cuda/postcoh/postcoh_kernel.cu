...
(
    ...
    int     cur_ifo_bits,           /* INPUT, all detectors that are in this coherent analysis */
...
)
{
    ...
    int srcLane = threadIdx.x & 0x1f, ipix, check_bit; // binary: 11111, decimal 31
    ...
    for (int ipeak = bid; ipeak < npeak; ipeak += bn)
    {
        ...
        for (int seed_pix = threadIdx.x; seed_pix < num_sky_directions/NSKY_REDUCE_RATIO; seed_pix += blockDim.x)
        {
			...
			if (cur_nifo == 2) {
                for (int k = 0; k < nifo; ++k) {
					check_bit = 1 << k;
					snr_tmp += (cur_ifo_bits & check_bit ?1:0) * (dk[k].re * dk[k].re + dk[k].im * dk[k].im);
				}
				al_all = 0.0;
