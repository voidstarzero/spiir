/*
 * Copyright (C) 2015 Yichun Li(buckfryspj@gmail.com), Yan Wang
 * (yan.wang@ligo.org), Qi Chu, Linqing Wen
 *
...
///////////////////////////////
// void ssvkernel(gsl_vector * x, gsl_vector * tin, gsl_vector *
// y_hist_result,gsl_matrix * result)
//
// input:
...
void pdf2cdf(PdfCdf *pc) {
    ...
    for (i = 0; i < pc->xn; i++) {
        for (j = 0; j < pc->yn; j++) { pc->pdf[i][j] = pc->pdf[i][j] / pdfSum; }
    }
    ...
    for (i = 0; i < x1->size; i++) {
        for (j = 0; j < x2->size; j++) {
            gsl_matrix_set(result, i, j,
                           gsl_vector_get(x1, i) * gsl_vector_get(x2, j));
        }
    }
}
double gsl_matrix_sum(gsl_matrix *x) {
    ...
    size_t i, j;
    for (i = 0; i < x->size1; i++) {
        for (j = 0; j < x->size2; j++) result += gsl_matrix_get(x, i, j);
    }
    return result;
}
long gsl_matrix_long_sum(gsl_matrix_long *x) {
    ...
    size_t i, j;
    for (i = 0; i < x->size1; i++) {
        for (j = 0; j < x->size2; j++) result += gsl_matrix_long_get(x, i, j);
    }
    return result;
}
...
void Gauss(gsl_vector *x, double w, gsl_vector *y) {
    // FIXME: two pointers pointing to the same address, might cause problems in
    // some case
...
void Boxcar(gsl_vector *x, gsl_vector *w, gsl_vector *y) {
    ...
    for (i = 0; i < x->size; i++) {
        ...
        if (fabs(t1) > t2 / 2) gsl_vector_set(y, i, 0);
        else
            ...
            if (g < gsl_vector_min(gs)) {
                ...
                for (i = 0; i < gs->size; i++) {
                    if (gsl_vector_get(gs, gs->size - 1 - i) >= g) {
                        gsl_vector_set(
                          optwv, k, g * gsl_vector_get(WIN, gs->size - 1 - i));
                        break;
                    }
                }
            }
...
void fftkernelWin(gsl_vector *data, double w, char *WinFunc, gsl_vector *y) {
    ...
    for (i = 0; i < n; i++) {
        if (i < L) X[i] = gsl_vector_get(data, i);
        else
            X[i] = 0;
    }
    ...
    if (strcmp(WinFunc, "Boxcar") == 0) {
        ...
        //		printf("Boxcar\n");
        double a = sqrt(12) * w;
        for (i = 0; i < n; i++) { K[i] = 2 * sin(a * t[i] / 2) / (a * t[i]); }
        K[0] = 1;
    ...
    for (i = 0; i < L; i++) { gsl_vector_set(y, i, REAL(data2, i)); }
}
void fftkernel(gsl_vector *data, double w, gsl_vector *y) {
    ...
    double f[n], K[n];
    for (i = 0; i < n / 2 + 1; i++) { f[i] = -(double)i / (double)n; }
    for (; i < n; i++) { f[i] = (double)(n - i) / (double)n; }
        ...
        for (l = 0; l < edges_dim1->size - 1; l++) {
            if (temp1 >= gsl_vector_get(edges_dim1, l)
                ...
                if (temp2 == gsl_vector_get(edges_dim2, edges_dim2->size - 1)) {
                    gsl_matrix_set(
                      result, l, edges_dim2->size - 1,
                      gsl_matrix_get(result, l, edges_dim2->size - 1) + 1);
                }
                break;
            }
        }
        if (temp1 == gsl_vector_get(edges_dim1, edges_dim1->size - 1)) {
            for (m = 0; m < edges_dim2->size - 1; m++) {
                if (temp2 >= gsl_vector_get(edges_dim2, m)
                    && temp2 < gsl_vector_get(edges_dim2, m + 1)) {
                    gsl_matrix_set(
                      result, edges_dim1->size - 1, m,
                      gsl_matrix_get(result, edges_dim1->size - 1, m) + 1);
                    break;
                }
            }
            if (temp2 == gsl_vector_get(edges_dim2, edges_dim2->size - 1)) {
                gsl_matrix_set(result, edges_dim1->size - 1,
                               edges_dim2->size - 1,
                               gsl_matrix_get(result, edges_dim1->size - 1,
                                              edges_dim2->size - 1)
                                 + 1);
            }
        }
...
void ssvkernel_from_hist(gsl_vector *y_hist_input,
                         gsl_vector *tin,
                         gsl_matrix *result) {
    ...
    for (i = 0; i < M; i++) {
        ...
        for (j = 0; j < L; j++) {
            gsl_matrix_get_col(C_local_col, C_local, j); // find optw at
                                                         // t=1....L
            gsl_matrix_set(
              optws, i, j,
    ...
    double f1 =
      CostFunction(y_hist, N, tin, dt, optws, WIN, WinFunc, c1, yv, optwp);
    double f2 =
      CostFunction(y_hist, N, tin, dt, optws, WIN, WinFunc, c2, yv, optwp);
...
void ssvkernel(gsl_vector *x,
               gsl_vector *tin,
               gsl_vector *y_hist_result,
               gsl_matrix *result) {
    size_t M = 80; // Number of bandwidths examined for optimization.
    ...
    for (i = 0; i < x_ab->size; i++) {
        if (gsl_vector_get(x_ab, i) < min_tin) gsl_vector_set(x_ab, i, min_tin);
        if (gsl_vector_get(x_ab, i) > max_tin) gsl_vector_set(x_ab, i, max_tin);
    }
    ...
    */
    ...
    double f1 =
      CostFunction(y_hist, N, t, dt, optws, WIN, WinFunc, c1, yv, optwp);
    double f2 =
      CostFunction(y_hist, N, t, dt, optws, WIN, WinFunc, c2, yv, optwp);
    //	printf("f1 = %.16lf, f2=%.16lf \n",f1,f2);
    size_t k = 1;
    while ((fabs(b - a) > tol * (fabs(c1) + fabs(c2))) && k < 30) {
        if (f1 < f2) {
