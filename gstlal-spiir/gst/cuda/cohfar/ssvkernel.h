 ...
 * (yan.wang@ligo.org)
 *
 *                                 This code converts pdf (probability density
 * function) of SNR and chi-squared to cdf (cumulative density function).
 *
 *                                  */
...
typedef struct {
    int xn, yn; // can be changed to lal/LALInspiral.h convesion if required in
                // the future
    double xtick[XN], ytick[YN];
    double xspac[XN], yspac[YN];
    // long double xyspac[xn][yn];
    long double pdf[XN][YN];
    long double cdf[XN][YN]; // can be changed to variable length if required in
                             // the future
} PdfCdf;
