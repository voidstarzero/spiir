...
__global__ void downsample2x(const float amplifier,
    ...
    for (i = tx; i < filt_len; i += tdx) tmp_sinc[i] = sinc[i];

    /* for the first block, get the head filt_len -1data from mem */
    if (bx < 1) {
        for (i = tx; i < filt_len - 1; i += tdx) tmp_mem[i] = mem_copy[i];
    ...
    if (tid < len) {
        ...
        tmp_mem[tx2 + filt_len - 1] =
          queue_in[(tid2 + last_sample_in) % len_in];
        tmp_mem[tx2 + filt_len] =
          queue_in[(tid2 + 1 + last_sample_in) % len_in];
    }
    ...
    for (i = tx; i < tail_len; i += tdx)
        tmp_mem[tmp_mem_end + filt_len - 1 + i] =
          queue_in[(in_end + i) % len_in];
    __syncthreads();
...
__global__ void
  cuda_iir_filter_kernel_coarse(COMPLEX_F *cudaA1,
                                COMPLEX_F *cudaB0,
                                int *cudaShift,
...
#endif
        fltrOutptReal =
          a1.re * previousSnr.re - a1.im * previousSnr.im + b0.re * data;
        fltrOutptImag =
          a1.re * previousSnr.im + a1.im * previousSnr.re + b0.im * data;
...
__global__ void cuda_iir_filter_kernel_fine(COMPLEX_F *cudaA1,
    ...
    // value of CU is specified by variable "cu", "logcu" is log2(cu)
    ...
    if (cuIdG < numTemplates) {
        ...
        for (int i = 0; i < len; ++i) {
#if __CUDA_ARCH__ >= 350
            data =
              __ldg(&cudaData[(shift + i + queue_first_sample) % queue_len]);
#else
            data = cudaData[(shift + i + queue_first_sample) % queue_len];
#endif
            fltrOutptReal =
            ...
            if (tIdL == 0) {
                cudaSnr[(2 * cuIdG + 0) * mem_len + filt_len - 1 + i] =
                  fltrOutptReal;
                cudaSnr[(2 * cuIdG + 1) * mem_len + filt_len - 1 + i] =
                  fltrOutptImag;
            }
        }
...
__global__ void upsample2x_and_add(float *sinc,
    ...
    unsigned int by = blockIdx.y;
    int pos, pos_in_start = mem_in_len * by + last_sample,
             pos_out_start = mem_out_len * by + filt_len - 1;
    float *in, *out;
    int i;
    for (i = tx; i < filt_len * 2; i += tdx) tmp_sinc[i] = sinc[i];
...
gint multi_downsample(SpiirState **spstate,
    ...
    if (SPSTATE(0)->queue_last_sample + num_inchunk <= SPSTATE(0)->queue_len) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(pos_inqueue, in_multidown,
                                   num_inchunk * sizeof(float),
                                   cudaMemcpyHostToDevice, stream));

    } else {

        int num_tail_samples =
          SPSTATE(0)->queue_len - SPSTATE(0)->queue_last_sample;
        pos_inqueue = SPSTATE(0)->d_queue + SPSTATE(0)->queue_last_sample;
        CUDA_CHECK(cudaMemcpyAsync(pos_inqueue, in_multidown,
                                   num_tail_samples * sizeof(float),
                                   cudaMemcpyHostToDevice, stream));

        int num_head_samples = num_inchunk - num_tail_samples;
        pos_inqueue          = SPSTATE(0)->d_queue;
        CUDA_CHECK(cudaMemcpyAsync(pos_inqueue, in_multidown + num_tail_samples,
                                   num_head_samples * sizeof(float),
                                   cudaMemcpyHostToDevice, stream));
    }
    ...
    for (i = 0; i < num_depths - 1; i++) {
        ...
        CUDA_CHECK(
          cudaMemcpyAsync(SPSTATEDOWN(i)->d_mem_copy, SPSTATEDOWN(i)->d_mem,
                          (SPSTATEDOWN(i)->sinc_len - 1) * sizeof(float),
                          cudaMemcpyDeviceToDevice, stream));
        downsample2x<<<grid, block, share_mem_sz, stream>>>(
          SPSTATEDOWN(i)->amplifier, 2, SPSTATEDOWN(i)->d_sinc_table,
          SPSTATEDOWN(i)->sinc_len, SPSTATEDOWN(i)->last_sample,
          SPSTATEDOWN(i)->d_mem, SPSTATEDOWN(i)->d_mem_copy, out_processed,
          pos_inqueue, SPSTATE(i)->queue_last_sample, SPSTATE(i)->queue_len,
        ...
        if (num_inchunk % 2 == 1)
            SPSTATE(i)->queue_last_sample =
              (SPSTATE(i)->queue_last_sample + num_inchunk - 1)
              % SPSTATE(i)->queue_len;
        else
            SPSTATE(i)->queue_last_sample =
              (SPSTATE(i)->queue_last_sample + num_inchunk)
              % SPSTATE(i)->queue_len;
...
gint spiirup(SpiirState **spstate,
    ...
    if (SPSTATE(i)->num_filters > CUT_FILTERS) {
        ...
        */
        ...
        if (SPSTATE(i)->num_filters > 32) {
            ...
            int spiir_block =
              ((numFilters + WARPSIZE - 1) >> LOGWARPSIZE) * WARPSIZE;
            CUDA_CHECK(cudaMemsetAsync(
              SPSTATEUP(i)->d_mem, 0,
              sizeof(COMPLEX_F) * mem_len * numTemplates, stream));
            cuda_iir_filter_kernel_coarse<<<spiir_grid, spiir_block, 0,
                                            stream>>>(
    ...
    SPSTATE(i)->queue_first_sample =
      (SPSTATE(i)->queue_first_sample + num_inchunk) % SPSTATE(i)->queue_len;
    ...
    for (i = num_depths - 2; i >= 0; i--) {
        ...
        if (SPSTATE(i)->num_filters > CUT_FILTERS) {
            ...
            */
            ...
            if (SPSTATE(i)->num_filters > 32) {
                ...
                int cuPerBlock  = spiir_block >> logcu;
                int spiir_grid  = (numTemplates + cuPerBlock - 1) / cuPerBlock;
                cuda_iir_filter_kernel_fine<<<spiir_grid, spiir_block, 0,
                                              stream>>>(
                  SPSTATE(i)->d_a1, SPSTATE(i)->d_b0, SPSTATE(i)->d_d,
                  SPSTATE(i)->d_y, SPSTATE(i)->d_queue, SPSTATEUP(i)->d_mem,
                  SPSTATEUP(i)->mem_len, SPSTATEUP(i)->filt_len,
        ...
        SPSTATE(i)->queue_first_sample =
          (SPSTATE(i)->queue_first_sample + spiir_processed)
          % SPSTATE(i)->queue_len;
        ...
        if (i == 0) {
            ...
            upsample2x_and_add_reshape<<<grid, block, share_mem_sz, stream>>>(
              SPSTATEUP(i + 1)->d_sinc_table, SPSTATEUP(i + 1)->filt_len,
              SPSTATEUP(i + 1)->last_sample, resample_processed,
            ...
            upsample2x_and_add<<<grid, block, share_mem_sz, stream>>>(
              SPSTATEUP(i + 1)->d_sinc_table, SPSTATEUP(i + 1)->filt_len,
              SPSTATEUP(i + 1)->last_sample, resample_processed,
              SPSTATEUP(i + 1)->d_mem, SPSTATEUP(i)->d_mem,
              SPSTATEUP(i + 1)->mem_len, SPSTATEUP(i)->mem_len);
    ...
    // CUDA_CHECK(cudaMemcpyAsync(out, SPSTATEUP(0)->d_mem,
    // SPSTATEUP(0)->channels * (SPSTATEUP(0)->mem_len) * sizeof(float),
    ...
    CUDA_CHECK(cudaMemcpyAsync(out, SPSTATE(0)->d_out,
